...
class SingleLeaveViewScreen extends StatefulWidget {
  ...
  const SingleLeaveViewScreen({Key? key, required this.leaveData})
      : super(key: key);
...
class _SingleLeaveViewScreenState extends State<SingleLeaveViewScreen> {
  ...
  Future<void> rejectLeave() async {
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Leave request reject!')),
      );
  ...
  void initState() {
    ...
    leaveIdController =
        TextEditingController(text: widget.leaveData['leaveId'] ?? '');
    nameController =
        TextEditingController(text: widget.leaveData['LeaveName'] ?? '');
    facultyController = TextEditingController(
        text: widget.leaveData['LectureFacultyNam'] ?? '');
    fromDateController =
        TextEditingController(text: widget.leaveData['LeaveFrom'] ?? '');
    toDateController =
        TextEditingController(text: widget.leaveData['LeaveTo'] ?? '');
    reasonsController =
        TextEditingController(text: widget.leaveData['LectureReason'] ?? '');
